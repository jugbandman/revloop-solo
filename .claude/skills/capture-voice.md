---
name: capture-voice
description: Voice calibration milestone. Harvests or paste-ingests the user's real writing samples and writes 60-Context/brand-voice.md. Invoked by /start or standalone.
---
...
3. **Write `60-Context/brand-voice.md`.** Use the existing schema in that file if present — preserve and extend. Append a `## Source Samples` section at the bottom containing the raw paste (lightly trimmed) so future recalibration has a reference.
...
Schema template: `60-Context/brand-voice.md` (if already present). Voice consumer: `.claude/agents/my-voice.md`.

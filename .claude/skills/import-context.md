...
- [ ] context/positioning.md exists and has real content
- [ ] context/icp.md exists and has real content
- [ ] context/brand-voice.md exists and has real content
- [ ] context/offerings.md exists and has real content
- [ ] context/competitors.md exists (nice-to-have)
- [ ] context/tool-stack.md exists (nice-to-have)

**Pipeline health:**
- [ ] pipeline/prospects/ has at least 1 entity with context.md
- [ ] pipeline/deals/ has at least 1 entity (if applicable)
- [ ] people/ has at least 5 contact files

**Operational health:**
- [ ] meetings/ has transcripts or notes
- [ ] playbooks/ has at least 1 playbook
- [ ] _imports/ has pending files to process
...
- **Import:** "Drop your pitch deck in _imports/ and I'll extract positioning"
- **Manual:** "I'll create the file with guided questions"
- **Skip:** "I'll do this later"

### Step 4: Process Any Pending _imports/

Check `_imports/` for unprocessed files. If found:
- Identify file types (transcript, CSV, PDF, markdown, pitch deck)
- Suggest routing for each:
  - Transcripts → meetings/ (offer to run /process-transcript)
  - CSVs → pipeline/ (offer to import as leads/prospects)
  - PDFs/docs → context/ (offer to summarize and extract)
  - Pitch decks → context/positioning.md + context/offerings.md
  - Brand guides → context/brand-voice.md
  - Playbooks → playbooks/
...
3. Create: context/brand-voice.md
...
3. Create files in playbooks/
4. Update _system/FEATURES.md if new processes are documented

**Transcripts:**
1. "Drop transcript files in _imports/ (or point to a folder)"
2. For each: run /process-transcript to extract summary, action items, entities
3. Route processed transcripts to meetings/
4. Create/update people files from attendees

**Contact list:**
1. "Drop the CSV or paste names"
2. Create entity files in people/ with available context
...
- **Watched folder route:** "Point your transcript tool's export folder at _imports/"
  - Create a launchd plist or cron job that runs `/process-transcript` on new files
  - Provide the exact command: `claude -p "Process any new transcripts in _imports/"`
  - Suggest cadence: every 30 minutes or on-demand
- **Manual route:** "Drop transcripts in _imports/ whenever. Run /import-context to process them."
...
- Or: set up a weekly CSV export → _imports/ pipeline
- Suggest: "Start with weekly CSV drops. Automate when the workflow is stable."

**Lead lists:**
- "Drop CSVs in _imports/ anytime"
- Optional: set up a Google Sheets sync via cron
- Provide command: `claude -p "Import leads from _imports/*.csv"`
...
*/30 * * * * cd /path/to/vault && claude -p "Check _imports/ for new transcripts and process them"
...
- Update `_system/health.md` automation status table

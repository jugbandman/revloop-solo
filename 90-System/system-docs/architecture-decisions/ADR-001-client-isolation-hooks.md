...
**Vault-level gate:** Block ALL writes until global Required Sources from `CLAUDE.md` (or a sacred sources checklist it references) are loaded. These are foundational files (60-Context/positioning.md, 60-Context/icp.md, 60-Context/brand-voice.md) that must inform every piece of output.
...
```
Write requested for: 80-Pipeline/prospects/Acme/outreach-draft.md

Vault gate: Check CLAUDE.md Sacred Sources
  ├── 60-Context/positioning.md loaded? YES
  ├── 60-Context/icp.md loaded? YES
  └── 60-Context/brand-voice.md loaded? YES
  → PASS

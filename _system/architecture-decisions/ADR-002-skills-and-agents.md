...
Both the scripts and any operator voice agents needed to be adapted for the revloop-solo vault, which is a standalone repo with its own folder structure (`pipeline/`, `context/`, `meetings/`, `Deliverables/`, `Workspace/`).
...
| Agent | Purpose |
|-------|---------|
| `my-voice` | Operator voice calibration — reads `context/brand-voice.md` and enforces voice rules on all client-facing output |
...
- `@company-context/` → `context/` (ICP, personas, offerings, brand voice)
- `@gtm-context/` → `context/strategy/` and `.revloop/templates/`
- `data/clients/` → `pipeline/prospects/`, `pipeline/deals/` (entity-level folders)
- `data/transcripts/` → `meetings/`
...
**Operator voice rules embedded.** All client-facing skills reference `context/brand-voice.md` and enforce the operator's voice rules. The `my-voice` agent can be invoked directly for voice review.
...
**process-transcript skill:** Takes raw meeting transcripts and extracts structured summaries, action items, buyer signals, and key quotes. Saves to `meetings/` and updates entity context.

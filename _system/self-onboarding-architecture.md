...
pipeline/            # prospects, deals, customers (differentiated by frontmatter)
people/              # individuals across all orgs
meetings/            # meeting recordings (internal/ + external/)
context/             # positioning, ICP, brand voice, playbooks, source materials
Deliverables/        # clean client-facing output
Workspace/           # scratch + WIP
++HOME/              # navigation MOCs

.revloop/
  system/            # vault self-awareness (FEATURES.md, health.md, ROADMAP.md)
...
Data folders (`pipeline/`, `people/`, `meetings/`, `context/`, `Workspace/`, `Deliverables/`) stay user-owned. The architecture never rewrites or moves data — only the scaffolding around it.
...
See `_system/fork-this-vault.md`.

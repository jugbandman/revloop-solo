...
#   - Required context/ files exist and lack placeholder markers
#   - .revloop/onboarding.json exists
#   - .env file exists
#   - .claude/hooks/*.sh are executable
#
# Usage:
#   bash _scripts/health-check.sh
#   ./_scripts/health-check.sh

set -uo pipefail

# Resolve vault root (one level up from _scripts/)
...
CTX_DIR="$VAULT_ROOT/context"
MISSING_REQ=0

for req in "${REQUIRED_FILES[@]}"; do
    filepath="$CTX_DIR/$req"
    if [ ! -f "$filepath" ]; then
        echo "  ✗ MISSING: context/$req"
        MISSING_REQ=$((MISSING_REQ + 1))
    else
        if grep -qE '\{add value\}|\{add date\}' "$filepath" 2>/dev/null; then
            echo "  ⚠ context/$req — contains placeholder markers (fill in before use)"
        else
            echo "  ✓ context/$req"
        fi
    fi

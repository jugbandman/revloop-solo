...
Draft or refine an enterprise account plan using the canonical template at `_templates/account-plan.md`. Deal-slug-driven. Idempotent — safe to re-run as new context lands.
...
Deal slug matches the folder name in `pipeline/deals/`. Examples: `acme-corp-discovery`, `contoso-qualifying`, `the-operator-buy-in`.

## Prerequisites

- `_templates/account-plan.md` exists and is current (canonical template).
- `pipeline/deals/{slug}/` exists with at least `context.md`.
- `context/positioning.md` and `context/icp.md` are up to date.
...
1. `_templates/account-plan.md` — canonical structure. Do not deviate from section order or heading text.
2. `pipeline/deals/{slug}/context.md` — entity context (company, stage, owner, key contacts, deal history, open questions).
3. `pipeline/deals/{slug}/60sec-review.md` — the 60-second deal review (if it exists).
4. `pipeline/deals/{slug}/business-case.md` — the business case (if it exists).
5. `pipeline/deals/{slug}/account-plan.md` — prior version, if present. Informs the idempotent update path below.
6. Relevant bowtie: check `pipeline/deals/` for any bowtie docs matching the deal owner.
7. `meetings/external/` — filter by filename containing the company name or slug root. Read all matching transcripts. These are the source for Deal History, buying committee details, and buyer-voiced success metrics.
8. `context/positioning.md` — your positioning. Use for the Why We Fit section.
9. `context/icp.md` — ICP and buyer personas. Cross-check account profile.
10. `context/brand-voice.md` — tone rules. Final pass checks compliance.

## Workflow

### 1. Resolve inputs

- Verify the deal folder exists. If not, stop and list available slugs in `pipeline/deals/`.
...
- Collect all matching transcripts in `meetings/external/` by company name substring.
...
Fill sections in this order. Each section pulls from specific inputs per `_templates/account-plan.md` comments.
...
- Path: `pipeline/deals/{slug}/account-plan.md`

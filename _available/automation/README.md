...
An Execute Command node calls a Python script (e.g., `_scripts/ingest-leads.py`) with the validated data. The script creates or updates a markdown file in the correct vault folder, writes frontmatter, and returns an exit code. n8n captures stdout and stderr.
...
- Your vault ingest scripts present in `_scripts/` (the sample calls `_scripts/ingest-leads.py`)
...
- **Meeting notes ingestion**: a Granola webhook posts transcript JSON, n8n formats it as markdown, the script saves to `meetings/`
- **CRM sync**: HubSpot deal updates fire a webhook, n8n writes status changes to `pipeline/deals/{slug}/context.md`
- **Enrichment callbacks**: Clay fires a webhook when enrichment completes, n8n routes the payload to `_available/enrichment/clay-enrich.py`
